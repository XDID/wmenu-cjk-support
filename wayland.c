...
void context_update_text_input(struct wl_context *context) {
	if (!context || !context->text_input || !context->text_input_focused) {
		return;
	}

	if (!context->text_input_enabled) {
		zwp_text_input_v3_enable(context->text_input);
		context->text_input_enabled = true;
	}

	struct menu *menu = context->menu;
	uint32_t hint = menu->passwd
		? ZWP_TEXT_INPUT_V3_CONTENT_HINT_HIDDEN_TEXT |
			ZWP_TEXT_INPUT_V3_CONTENT_HINT_SENSITIVE_DATA
		: ZWP_TEXT_INPUT_V3_CONTENT_HINT_NONE;
	uint32_t purpose = menu->passwd
		? ZWP_TEXT_INPUT_V3_CONTENT_PURPOSE_PASSWORD
		: ZWP_TEXT_INPUT_V3_CONTENT_PURPOSE_NORMAL;

	zwp_text_input_v3_set_content_type(context->text_input, hint, purpose);
	zwp_text_input_v3_set_surrounding_text(context->text_input, menu->input,
		(int32_t)menu->cursor, (int32_t)menu->cursor);
	zwp_text_input_v3_set_cursor_rectangle(context->text_input, menu->promptw, 0,
		menu->line_height, menu->line_height);
	zwp_text_input_v3_commit(context->text_input);
	context->text_input_serial++;
}
...
static void context_destroy(struct wl_context *context) {
	...
	if (context->text_input) {
		zwp_text_input_v3_destroy(context->text_input);
	}
	if (context->text_input_manager) {
		zwp_text_input_manager_v3_destroy(context->text_input_manager);
	}
	...
	free(context->pending_commit);
	free(context->pending_preedit);
...
static void text_input_enter(void *data, struct zwp_text_input_v3 *text_input,
	...
	context->text_input_focused = true;
	context_update_text_input(context);
}
...
static void text_input_leave(void *data, struct zwp_text_input_v3 *text_input,
	...
	context->text_input_focused = false;
	context->text_input_enabled = false;
	free(context->pending_commit);
	context->pending_commit = NULL;
	free(context->pending_preedit);
	context->pending_preedit = NULL;
	context->pending_delete = false;
	menu_apply_text_input(context->menu, 0, 0, NULL, NULL, -1, -1);
}
...
static void text_input_preedit_string(void *data,
	...
	(void)cursor_begin;
	free(context->pending_preedit);
	context->pending_preedit = text ? strdup(text) : NULL;
	context->menu->preedit_cursor = cursor_end >= 0 ? (size_t)cursor_end : 0;
	context->menu->preedit_cursor_visible = cursor_begin >= 0 && cursor_end >= 0;
}
...
static void text_input_commit_string(void *data,
	...
	struct wl_context *context = data;
	free(context->pending_commit);
	context->pending_commit = text ? strdup(text) : NULL;
}

static void text_input_delete_surrounding_text(void *data,
		struct zwp_text_input_v3 *text_input, uint32_t before_length,
		uint32_t after_length) {
	struct wl_context *context = data;
	context->pending_delete_before = before_length;
	context->pending_delete_after = after_length;
	context->pending_delete = true;
}
...
static void text_input_done(void *data, struct zwp_text_input_v3 *text_input,
	...
	struct wl_context *context = data;
	(void)serial;
	int32_t preedit_cursor = context->menu->preedit_cursor_visible
		? (int32_t)context->menu->preedit_cursor : -1;

	menu_apply_text_input(context->menu,
		context->pending_delete ? context->pending_delete_before : 0,
		context->pending_delete ? context->pending_delete_after : 0,
		context->pending_commit, context->pending_preedit,
	...
	free(context->pending_preedit);
	context->pending_preedit = NULL;
	context->pending_delete_before = 0;
	context->pending_delete_after = 0;
	context->pending_delete = false;
}
...
static void handle_global(void *data, struct wl_registry *registry,
	...
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		context->text_input_manager = wl_registry_bind(registry, name,
...
int menu_run(struct menu *menu) {
	...
	assert(context->text_input_manager != NULL);
	...
	context->text_input = zwp_text_input_manager_v3_get_text_input(
		context->text_input_manager, context->seat);
	zwp_text_input_v3_add_listener(context->text_input, &text_input_listener, context);
