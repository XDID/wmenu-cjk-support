...
void calc_widths(struct menu *menu) {
	...
	if (menu->prompt) {
		menu->promptw = text_width(cairo, menu->font, menu->latin_font,
			menu->cjk_font, menu->prompt) + menu->padding + menu->padding/2;
	} else {
		menu->promptw = 0;
	}

	// Calculate scroll indicator widths
	menu->left_arrow = text_width(cairo, menu->font, menu->latin_font,
		menu->cjk_font, "<") + 2 * menu->padding;
	menu->right_arrow = text_width(cairo, menu->font, menu->latin_font,
		menu->cjk_font, ">") + 2 * menu->padding;

	// Calculate item widths and input area width
	for (size_t i = 0; i < menu->item_count; i++) {
		struct item *item = &menu->items[i];
		item->width = text_width(cairo, menu->font, menu->latin_font,
			menu->cjk_font, item->text);
...
static int render_text(struct menu *menu, cairo_t *cairo, const char *str,
	...
	get_text_size(cairo, menu->font, menu->latin_font, menu->cjk_font,
		&text_width, &text_height, NULL, 1, str);
	...
	pango_printf(cairo, menu->font, menu->latin_font, menu->cjk_font, 1, str);
...
static size_t build_display_input(struct menu *menu, char *display, size_t size) {
	size_t cursor = menu->cursor;
	if (menu->preedit[0] && !menu->passwd) {
		snprintf(display, size, "%.*s%s%s",
			(int)menu->cursor, menu->input, menu->preedit, menu->input + menu->cursor);
		if (menu->preedit_cursor_visible) {
			cursor += menu->preedit_cursor;
		} else {
			cursor = SIZE_MAX;
		}
	} else {
		strncpy(display, menu->input, size - 1);
		display[size - 1] = '\0';
	}
	return cursor;
}
...
static void render_input(struct menu *menu, cairo_t *cairo) {
	char *censort = NULL;
	char display[sizeof(menu->input) + sizeof(menu->preedit)];
	...
	if (menu->passwd) {
		...
		strncpy(display, censort, sizeof(display) - 1);
		display[sizeof(display) - 1] = '\0';
	} else {
		build_display_input(menu, display, sizeof(display));
	}

	render_text(menu, cairo, display,
...
static void render_cursor(struct menu *menu, cairo_t *cairo) {
	...
	if (menu->passwd) {
		cursor = menu->cursor;
		memset(display, '*', menu->cursor);
		strcpy(display + menu->cursor, menu->input + menu->cursor);
		for (size_t i = menu->cursor; menu->input[i]; i++) {
			display[i] = '*';
		}
		display[strlen(menu->input)] = '\0';
	} else {
		cursor = build_display_input(menu, display, sizeof(display));
	}
	if (cursor == SIZE_MAX) {
		return;
	}
	const char *suffix = display + cursor;
	int cursor_pos = menu->promptw + menu->padding
		+ text_width(cairo, menu->font, menu->latin_font, menu->cjk_font, display)
		- text_width(cairo, menu->font, menu->latin_font, menu->cjk_font, suffix)
		- cursor_width / 2;
...
static void render_horizontal_page(struct menu *menu, cairo_t *cairo, struct page *page) {
	...
	if (page->prev) {
		...
		pango_printf(cairo, menu->font, menu->latin_font, menu->cjk_font, 1, "<");
	}
	if (page->next) {
